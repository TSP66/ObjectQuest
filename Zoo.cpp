...
std::string Zoo::summary(std::string name, int money, int day,
                         DailySales dailySales) {
  std::string message =
      "\n" + GREEN + name + " zoo" + RESET + " (Day: " + std::to_string(day) +
      ", Funds: $" + std::to_string(money) +
      ", Visitors: " + std::to_string(dailySales.visitors) + ", Sales: $" +
      std::to_string(dailySales.revenue) + ", Debt: $" +
      std::to_string(Zoo::bank.getTotal()) + ")\n";

  bool zooEmpty = true;

  for (int enclosureId : Zoo::enclosureIds) {
    message +=
        INDENT + MAGENTA + "Enclosure" + RESET +
        " (type: " + Zoo::enclosures[enclosureId]->get_name() +
    ...
    for (auto mapObject : Zoo::enclosures[enclosureId]->animals) {
      message +=
          INDENT + INDENT + CYAN + mapObject.second->get_name() + RESET  // name
          +
          " (age: " + std::to_string((int)round(mapObject.second->get_age())) +
          ", sex: " + sexToString(mapObject.second->get_sex()) +
          ", happiness: " +
          std::to_string((int)round(mapObject.second->get_happiness())) +
...
DailySales Zoo::getRevenue() {
  ...
  for (auto mapObject : Zoo::animals) {
    double animalScore =
        pow((double)mapObject.second->get_cost() / 25.0,
            2.0);  // Exotic Animals are more expensive but also more popular.

    if (mapObject.second->get_age() <
        1.0) {  // checks to see if animal is baby and if so acounts for that
                // with revenue
  ...
  // visitors is used to determine the amount of revenue the zoo makes per turn
  // (week)
  int visitors = (int)(250.0 + 750.0 * weather_factor) *
                 exp(-expodent * (double)Zoo::ticketPrice) *
                 (log10(zooScore + 10.0) - 1.0);
...
Changes Zoo::buyAnimal(int money) {
  ...
  int choice = Zoo::displayOptions(
      Zoo::animalInformation);  // displayes animal options e.g lion and dolphin
  AnimalInformation parameters = Zoo::animalInformation[choice];

  if (parameters.cost > money) {  // if you dont have enough money "Insufficent
...
#undef XX
  ...
  int numEnclosures =
      Zoo::enclosureIds.size();  // determins the number of enclosures
  ...
  for (int i = 0; i < numEnclosures; i++) {
    ...
    if (Zoo::enclosures[id]->enclosureType == parameters.enclosureType) {
      // std::cout << INDENT << i << ": " << MAGENTA <<
      // Zoo::enclosures[id]->get_name() << RESET <<" | " <<
      // Zoo::enclosures[id]->get_id() << "\n";.
      enclosureNames.push_back(Zoo::enclosures[id]->get_name() + " | " +
                               std::to_string(Zoo::enclosures[id]->get_id()));
  ...
  int selection = optionSelector(
      enclosureNames,
      std::string("Okay, which enclosure would you like to put it in: "), true);
  int enclosureChoice = suitableEnclosures[selection];
  // Create shared pointer.
  std::shared_ptr<Animal> newAnimalPtr =
      std::shared_ptr<Animal>(std::move(newAnimal));

  // Add pointer to enclosure object.
  if (!Zoo::enclosures[Zoo::enclosureIds[enclosureChoice]]->addAnimal(
          id, newAnimalPtr)) {
  ...
  return {parameters.cost, true,
          std::string("Successfully added animal to enclosure!"), GREEN};
}

Changes Zoo::buildEnclosure(int money) {  // displays the enclosure information
                                          // and allows user to opick which
  ...
  return {parameters.cost, true, std::string("Successfully added enclosure!"),
          GREEN};
}
...
void Zoo::addEnclosure(EnclosureInformation parameters) {
  ...
  switch (parameters.type) {
    case LAND:  // creates a different enclosure depending on the enclosure type
      newEnclosure =
          new LandEnclosure(parameters.name, parameters.maxAnimals);  // land
      break;

    case AQUATIC:
      newEnclosure =
          new Tank(parameters.name, parameters.maxAnimals);  // aquatic
      break;
  }

  std::unique_ptr<Enclosure> newEnclosureUP =
      std::unique_ptr<Enclosure>(newEnclosure);
...
// diasplays animal information such as price and name and what enclosure type
// they require.
std::vector<AnimalInformation> Zoo::makeAnimalInformation() {
  std::vector<AnimalInformation> animalInformation{
#define XX(NAME) \
  AnimalInformation(#NAME, PRICE_ANIMAL(NAME), CAPITALIZE_ANIMAL(NAME), LAND),
      LAND_ANIMAL_LIST(XX)
#undef XX
#define XX(NAME)                                                        \
  AnimalInformation(#NAME, PRICE_ANIMAL(NAME), CAPITALIZE_ANIMAL(NAME), \
                    AQUATIC),
...
template int Zoo::displayOptions<EnclosureInformation>(
    std::vector<EnclosureInformation>);
template int Zoo::displayOptions<AnimalInformation>(
    std::vector<AnimalInformation>);
// template int
// Zoo::displayOptions<UtilitiesInformation>(std::vector<UtilitiesInformation>);.

int Zoo::ticketPriceFromOption(int option) {
  return atoi(Zoo::ticketPriceChoices[option]
                  .substr(1, Zoo::ticketPriceChoices.size() - 1)
                  .c_str());
}
...
Changes Zoo::setTicketPrice() {
  ...
  std::cout << "Your current ticket price is $" << Zoo::ticketPrice
            << ", changing it will cost $100\nNew price (ENTER to keep old): ";

  // allows the user to enter a ticket price of their choice
  // this changes customer appearances depending on if the ticket is of a fair
  // price
  int choice = optionSelector(Zoo::ticketPriceChoices,
                              std::string("Enter a new ticket price: "), true);
  ...
  if (newTicketPrice == Zoo::ticketPrice)  // if ticket price isnt changed
    ...
    Zoo::ticketPrice =
        newTicketPrice;  // displayed when ticket is successfully changed
    return {100, true, std::string("Successfully changed ticket price!"),
            GREEN};
  }
}
...
std::string Zoo::ageAnimals() {
  ...
  for (auto mapObject : Zoo::animals) {
    ...
    if (death != NONE) {  // what is displayed when an animal dies
      message +=
          RED + "† A " + mapObject.second->get_name() + RESET +
          " has died (Age: " +
          std::to_string((int)round(mapObject.second->get_age())) +
...
Changes Zoo::feedAnimal(int money) {  // feeding animals
  ...
  if (numAnimals == 0) {
    return {0, true, std::string("You have no animals!"),
            RED};  // if you try to feed an animal without any animals bought
  }
  ...
  for (int i = 0; i < numAnimals; i++) {
    ...
    animalsToFeed.push_back(
        Zoo::animals[Id]->get_name() + "(cost to feed: $" +
        std::to_string(round(Zoo::animals[Id]->get_hunger() / 3.0 *
                             Zoo::animals[Id]->get_cost() / 10.0)) +
        ", hunger: " + std::to_string(Zoo::animals[Id]->get_hunger()) +
        ", happiness: " + std::to_string(Zoo::animals[Id]->get_happiness()) +
        ", id: " + std::to_string(Id));

    /*
    std::cout << INDENT << i << ": " << BLUE << Zoo::animals[Id]->get_name() <<
    RESET //name.
                  << " (cost to feed: $" << (int)
    round(Zoo::animals[Id]->get_hu.nger()/3.0 *
    Zoo::animals[Id]->get_cost()/10.0)
  ...
  int choice = optionSelector(
      animalsToFeed, std::string("Which Animal would you like to feed: "),
      true);

  int Id = Zoo::animalIds[choice];
  int cost = (int)round(Zoo::animals[Id]->get_hunger() / 10.0 *
                        Zoo::animals[Id]->get_cost() / 10.0);
  ...
  Zoo::animals[Id]->set_happiness(Zoo::animals[Id]->get_happiness() +
                                  100.0);  // Increase happiness by 100.
...
Changes Zoo::sellAnimal(void) {  // function used to sell animals
  ...
  if (numAnimals == 0) {
    return {
        0, true, std::string("You have no animals!"),
        RED};  // used if you try to sell animals without any present in the zoo
  }
  ...
  for (int i = 0; i < numAnimals; i++) {
    ...
    animalsToSell.push_back(
        Zoo::animals[Id]->get_name()  // shoes the value of the animal that your
                                      // trying to sell and the animasl ID
        + "(value: $" +
        std::to_string(std::max(1.0, Zoo::animals[Id]->get_cost() -
                                         3.0 * Zoo::animals[Id]->get_age())) +
        ", id: " + std::to_string(Id));

    /*
    std::cout << INDENT << i << ": " << BLUE << Zoo::animals[Id]->get_name() <<
    RESET//name.
                  << " (value: $" << (int) std::max(1.0,
    Zoo::animals[Id]->get_cost() - 3.0 * Zoo::an.imals[Id]->get_age())
                  << ", id: " << Id << ")\n";.
    */
  }

  int choice = optionSelector(
      animalsToSell, std::string("Which Animal would you like to sell: "),
      true);  // allows the user to choose which animals to sell

  int Id = Zoo::animalIds[choice];
  int value = (int)std::max(
      1.0, Zoo::animals[Id]->get_cost() - 3.0 * Zoo::animals[Id]->get_age());
...
Changes Zoo::moveAnimal(void) {
  ...
  if (numAnimals == 0) {
    return {0, true, std::string("You have no animals!"),
            RED};  // displays when you try to move an animal when no animals
                   // are in the zoo
  }

  // asks the user which animal they wish to move. This only occurs when there
  // are animals that
  ...
  for (int i = 0; i < numAnimals; i++) {
    ...
    animalsToMove.push_back(
        Zoo::animals[Id]->get_name() +
  ...
  int choice = optionSelector(
      animalsToMove, std::string("Which Animal would you like to move: "),
      true);
  //
  ...
  for (int i = 0; i < Zoo::enclosureIds.size(); i++) {
    int id = Zoo::enclosureIds[i];
    if (Zoo::enclosures[id]->enclosureType ==
        Zoo::enclosures[oldEnclosureId]->enclosureType) {
      suitableEnclosuresNames.push_back(
          Zoo::enclosures[id]->get_name() + " | " +
          std::to_string(Zoo::enclosures[id]->get_id()));
      // std::cout << INDENT << i << ": " << MAGENTA <<
      // Zoo::enclosures[id]->get_name() << RESET << " | " <<
      // Zoo::enclosures[id]->get_id() << "\n";
  ...
  if (suitableEnclosures.size() <= 1) {
    ...
    return {0, true, std::string("You have no other suitable enclosures!"),
            RED};
  }

  int enclosureChoice = suitableEnclosures[optionSelector(
      suitableEnclosuresNames,
      std::string("Okay, which enclosure would you like to put it in: "),
      true)];

  if (!Zoo::enclosures[Zoo::enclosureIds[enclosureChoice]]->addAnimal(
          animalId, Zoo::animals.at(animalId))) {
    // std::cout << RED << "Enclosure has no space!\n" << RESET;
    // delete newAnimalPtr; //Free memory if can't make it
    return {0, true, std::string("Enclosure has no space!"), RED};
  }
...
Changes Zoo::goToBank(int money) {
  ...
  int choice =
      optionSelector(bankingOptions, std::string("Welcome to the bank!"), true);

  if (choice == 0) {  // Pay Loan Option.
    if (Zoo::bank.getNLoans() > 0) {
      int loanIndex = optionSelector(
          Zoo::bank.getLoanInfo(),
          std::string("Which loan would you like to pay off?"), true);
    ...
    if (!Zoo::bank.checkCredit(money)) {
      return {0, true,
              std::string("Your credit is too bad! You can't borrow more!"),
              RED};
    }
    int loanAmountChoice =
        optionSelector(Zoo::bank.loanAmounts,
                       std::string("How much would you like to borrow? (Higher "
                                   "amount = higher interest)"),
                       true);
    ...
    return {-loanAmount, true,
            std::string("Successfully borrowed $" + std::to_string(loanAmount) +
                        "!"),
            GREEN};
  }
